...
void RK45_update_kernel_wrapper(int in_n_upts_per_ele,int in_n_dims,int in_n_fields,int in_n_eles,double* in_disu0_upts_ptr,double* in_disu1_upts_ptr,double* in_div_tconf_upts_ptr, double* in_detjac_upts_ptr, double* in_src_term, double in_rk4a, double in_rk4b, double in_dt, double in_const_src_term);

void RK11_update_kernel_wrapper(int in_n_upts_per_ele,int in_n_dims,int in_n_fields,int in_n_eles,double* in_disu0_upts_ptr,double* in_div_tconf_upts_ptr, double* in_detjac_upts_ptr, double* in_src_term, double in_dt, double in_const_src_term);
...
void calc_tdisvisf_upts_gpu_kernel_wrapper(int in_n_upts_per_ele, int in_n_dims, int in_n_fields, int in_n_eles, double* in_disu_upts_ptr, double* out_tdisvisf_upts_ptr, double* in_grad_disu_upts_ptr, double* in_detjac_upts_ptr, double* in_inv_detjac_mul_jac_upts_ptr, double in_gamma, double in_prandtl, double in_rt_inf, double in_mu_inf, double in_c_sth, double in_fix_vis, int equation, double diff_coeff, int artif_only, double* in_epsilon_ptr, double* in_epsilon_upts_ptr, int in_turb_model, double in_c_v1, double in_omega, double in_prandtl_t);

void calc_src_term_SA_gpu_kernel_wrapper(int n_upts_per_ele, int n_dims, int n_fields, int n_eles, double* in_disu_upts_ptr, double* grad_disu_upts_ptr, double* wall_distance_mag_ptr, double* src_term_ptr, double in_gamma, double in_prandtl, double in_rt_inf, double in_mu_inf, double in_c_sth, int in_fix_vis, double in_c_v1, double in_c_v2, double in_c_v3, double in_c_b1, double in_c_b2, double in_c_w2, double in_c_w3, double in_omega, double in_Kappa);
...
/*! wrapper for gpu kernel to calculate artificial viscosity co-efficients */
void calc_artivisc_coeff_gpu_kernel_wrapper(int, int, int, int, double, double, double, double*, double*, double*, double*, int*);

/*! wrapper for gpu kernel to correct density */
void correct_rho_gpu_kernel_wrapper(int, int, double*);

/*! wrapper for gpu kernel to calculate artificial viscosity co-efficients at vertices -called from mesh */
void calc_artivisc_coeff_verts_gpu_kernel_wrapper(int in_num_verts, int* in_icvert, int* icvsta, double* in_epsilon_global_eles, double* out_epsilon_verts);

/*! wrapper for gpu kernel to calculate AV coeffs at solution points */
