...
template<int in_n_fields, int in_n_dims>
__device__ __host__ void convective_flux_boundary(double* q_l, double *q_r, double *norm, double *fn, double in_gamma, int in_turb_model)
{
  double vn_l, vn_r;
  double vn_av_mag, c_av;
  double p_l, p_r,f_l,f_r;
...
#pragma unroll
      ...
      if (bdy_spec==16) // Dual consistent
        ...
        {
          if (in_riemann_solve_type==0)
            convective_flux_boundary<in_n_fields,in_n_dims> (q_l,q_r,norm,fn,in_gamma,in_turb_model);
