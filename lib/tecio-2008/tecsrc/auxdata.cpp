...
/**
 * Deallocates an auxiliary data item and it's contents and sets the target to
 * NULL.
 *
 * param AuxDataItem
...
/**
 * Duplicates an auxiliary data item if it's Retain flag is TRUE or if directed
...
/**
 * Copies the auxiliary data and all it's members who's Retain flag is TRUE
      ...
      while (low <= high)
        {
          ...
          if (CompareResult < 0)
            ...
            low = ++Index; /* If the new name it "greater" than the one we're comparing
                              against, we want to make sure it's Index is greater than
