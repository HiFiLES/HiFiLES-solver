...
#define TecplotBinaryFileVersion    111   /* NOTE: MUST also change this define symbol in preplot.cpp */
/* Also change version number in the data file format in the comments in preplot.cpp */
/* And the version number of Preplot itself in preplot.cpp */
#define TecplotInterfaceVersion     112
#define TecplotInterfaceVersionStr  "112" /* stay in lockstep with TecplotInterfaceVersion */
#define TecplotLicenseVersion       112   /* may vary from TecplotInterfaceVersion */
#define TecplotLicenseVersionStr   "11.2"  /* stay in lockstep with TecplotLicenseVersion */
...
#   define SSTAT    ::_stat // ...the IBM already defines "STAT" so we use "SSTAT" instead
...
#  define FFLUSH   fflush
  ...
  #if defined _WIN64
    #define FSEEK(stream,offset,whence) _fseeki64((stream),(__int64)(offset),(whence))
...
#  define FSEEK    fseek
#  define FTELL    ftell
#  define SSTAT    stat // ...the IBM already defines "STAT" so we use "SSTAT" instead
...
/* BEGINREMOVEFROMADDON */
    ...
    SetValue_Ignored = SetValue_DuplicateValue,
/* ENDREMOVEFROMADDON */
...
typedef Boolean_t (*SuffixModifier_pf)(double     *Value,
                                       const char *Suffix);
...
typedef struct _NodeNormalCache_a   *NodeNormalCache_pa;
/* ENDREMOVEFROMADDON */
 ...
 * classic plus formats are identical. All values are layed out contiguously
...
typedef struct _ZoneSpec_s
  {
    ...
    /* classic data only */
    LgIndex_t          UserDefFNConnections;
    FaceNeighborMode_e FNMode;
    Boolean_t          AutoAssignFN; // ...classic FE only
 ...
 *   11.0-5-014
 */
typedef Boolean_t (STDCALL *PageCreateCallback_pf)(StringList_pa PageConstructionHints, ArbParam_t  RegistrationClientData);
 ...
 *
 * @param ImageHandle handle to an offscreen image fom which RGB values to be retrieved.
 *
 * @param Row row for which RGB values to be retrieved.
 *
 * @param RedArray preallocated array to hold Red values. Array should be allocated to hold number 
 * of values equal to the Width argument with which OffscreenImageCreateCallback_pf was called.
 *
 * @param GreenArray preallocated array to hold Green values. Array should be allocated to hold number 
 * of values equal to the Width argument OffscreenImageCreateCallback_pf was called.
 *
 * @param BlueArray preallocated array to hold Blue values. Array should be allocated to hold number 
 * of values equal to the Width argument OffscreenImageCreateCallback_pf was called.
 *
 * @param RegistrationClientData
 *   Data associated with the registration of this function.   This will always return
 ...
 *   11.2-0-054
 */
typedef void (STDCALL *OffscreenImageGetRGBRowCallback_pf)(ArbParam_t   ImageHandle,
                                                           ScreenDim_t  Row,
                                                           ArbParam_t   RegistrationClientData,
                                                           Byte_t     * RedArray,   /* OUT */
                                                           Byte_t     * GreenArray, /* OUT */
                                                           Byte_t     * BlueArray); /* OUT */
...
typedef void (STDCALL *KeyStateCallback_pf)(ArbParam_t  RegistrationClientData,
                                            Boolean_t  *IsShiftKeyDown,
                                            Boolean_t  *IsAltKeyDown,
                                            Boolean_t  *IsCntrlKeyDown);
...
typedef void (STDCALL *DotPitchCallback_pf)(ArbParam_t  RegistrationClientData,
                                            double     *IDotsPerCm,  /* OUT */
                                            double     *JDotsPerCm); /* OUT */
...
typedef void (STDCALL *ScreenSizeCallback_pf)(ArbParam_t  RegistrationClientData,
                                              int        *WidthInPixels,   /* OUT */
                                              int        *HeightInPixels); /* OUT */
...
typedef void (STDCALL *ProgressMonitorCallback_pf)(int          ProgressStatus,
                                                   ArbParam_t   RegistrationClientData);
/* - NO DOXYGEN COMMENT GENERATION -
 * Callback that will be called with Tecplot Engine is about to perform a lenghty operation.
 ...
 *
 * @par Note:
 *   This callback is called asynchronously. This callback must NOT lock/unlock
 *   Tecplot.
 *
 * @since
 *   10.0-3-128
 ...
 *   should be limited to queries. In addition, this callback must NOT
 *   lock/unlock Tecplot.
 *
 * @since
 *   11.0-0-001
 ...
 * Tecplot to handle the unloading (and subseqent reloading) of the variable
 * without the intervention of the add-on.
 *
 * @par Note:
 *   This callback is called asynchronously. All calls back to Tecplot through
 *   the TecUtil layer should be limited to queries. In addition, this callback
 *   must NOT lock/unlock Tecplot.
 *
 * @since
 *   11.0-0-001
 *
 ...
 *   layer should be limited to queries. In addition, this callback must NOT
 *   lock/unlock Tecplot.
 *
 * @since
 *   11.2-1-0
 ...
 *
 * Most add-ons should simply supply NULL for this callback thereby instructing
 * Tecplot to handle the unloading (and subseqent reloading) of the face mapping
 * without the intervention of the add-on.
 *
 * @par Note:
 *   This callback is called asynchronously. All calls back to Tecplot through
