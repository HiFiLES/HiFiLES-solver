                           ...
                           SmInteger_t  ItemSize)
{
  ...
  if (&TargetArray[TargetOffset] < &SourceArray[SourceOffset])
    {
      char *TargetPtr = &TargetArray[TargetOffset * ItemSize];
      char *SourcePtr = &SourceArray[SourceOffset * ItemSize];
      for (Index = 0; Index < CharCount; Index++, TargetPtr++, SourcePtr++)
        *TargetPtr = *SourcePtr;
    }
  else
    {
      char *TargetPtr = &TargetArray[TargetOffset * ItemSize + CharCount - 1];
      char *SourcePtr = &SourceArray[SourceOffset * ItemSize + CharCount - 1];
      for (Index = 0; Index < CharCount; Index++, TargetPtr--, SourcePtr--)
        *TargetPtr = *SourcePtr;
    }
}
 ...
 *
 * param ArrayList
 *     Array list needing it's items destroyed.
 * param ItemOffset
 ...
 *
 * param TargetArray
 *     Target array needing it's items duplicated.
 * param TargetItemOffset
 *     Target offset to the first duplicated item.
 * param SourceArray
 *     Source array needing it's items duplicated.
  ...
  if (Result != NULL)
    {
      ...
      if (ItemDuplicator != NULL)
        ...
        CopyArrayItems((char *)Result, 0,
                       ...
                       Source->ItemSize);
      if (!IsOk)
        FREE_ARRAY(Result, "native array");
    }
                    ...
                    ArbParam_t                  ClientData)
{
  ...
  #if defined TECPLOTKERNEL
    REQUIRE(Thread_IsMainline()); /* ...static state is used to hold the client data */
  #endif
  REQUIRE(ArrayListIsValid(ArrayList));
