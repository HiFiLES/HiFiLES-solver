      ...
      if (!IsStructureReferenced(*FieldData))
        {
          ...
          #if defined TECPLOTKERNEL
          if ((*FieldData)->LODInfo.IsReadableLock != NULL)
            Thread_FreeMutex(&(*FieldData)->LODInfo.IsReadableLock);
          if ((*FieldData)->LODInfo.IsWritableLock != NULL)
            Thread_FreeMutex(&(*FieldData)->LODInfo.IsWritableLock);
          #endif
